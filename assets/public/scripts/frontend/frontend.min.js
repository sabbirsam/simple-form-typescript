  ...
  formData.forEach(function (field) {
    ...
    switch (field.type) {
      ...
      case 'url':
        ...
        break;
      case 'image':
        html += "\n        <div class=\"text-fields\">\n          <label for=\"".concat(field.id, "\">").concat(field.label, "</label>\n          <input id=\"").concat(field.id, "\" type=\"").concat(field.type, "\" src=\"").concat(field.src, "\" alt=\"").concat(field.alt, "\" width=\"").concat(field.width, "\" height=\"").concat(field.height || '', "\">\n        </div>\n        ");
        break;
      case 'hidden':
        html += "\n            <input id=\"".concat(field.id, "\" type=\"").concat(field.type, "\" name=\"").concat(field.name, "\" placeholder=\"").concat(field.placeholder, "\" class=\"").concat(field.className, "\" value=\"").concat(field.value || '', "\"").concat(fieldAttributes, ">\n        ");
        break;
      case 'textarea':
        html += "\n            <div class=\"text-fields\">\n              <label for=\"".concat(field.id, "\">").concat(field.label, "</label>\n              <textarea type=\"").concat(field.type, "\" placeholder=\"").concat(field.placeholder, "\" class=\"").concat(field.className, "\" ").concat(fieldAttributes, " id=\"").concat(field.id, "\" name=\"").concat(field.name, "\" data-unique-id=\"").concat(field.uniqueId, "\" subtype=\"type=\"").concat(field.type, "\"></textarea>\n            </div>\n        ");
        break;
      case 'select':
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            /* eslint-disable */
            (_console = console).log.apply(_console, _toConsumableArray(oo_oo("194972901_280_12_280_50_4", 'Response:', responseData)));
            if (responseData.success) {
               ...
               */
              var scf_json = getCookie("simple_form_whatsapp_data");
              if (scf_json) {
                var scf_opt = JSON.parse(scf_json);
                if (scf_opt && scf_opt.simple_form_whatsapp_number) {
                  var newTab = scf_opt.simple_form_new_tab === "true";
                  var target = newTab ? "_blank" : "_self";
                  var number = scf_opt.simple_form_whatsapp_number;

                  // Convert the simple_form_whatsapp_data object to a formatted string
                  var text = Object.keys(scf_opt.simple_form_whatsapp_data).map(function (key) {
                    return "".concat(key, ": ").concat(scf_opt.simple_form_whatsapp_data[key]);
                  }).join("\n");
                  var mobileurl = "https://wa.me/".concat(number, "?text=").concat(encodeURIComponent(text));
                  var weburl = "https://web.whatsapp.com/send?phone=".concat(number, "&text=").concat(encodeURIComponent(text));
                  var url = window.innerWidth > 1024 ? weburl : mobileurl;
              ...
              var _console2;
              /* eslint-disable */(_console2 = console).log.apply(_console2, _toConsumableArray(oo_oo("194972901_333_14_333_52_4", responseData.data.message)));
...
function restartAnimation() {
  var whatsappIcon = document.getElementById('jumping-whatsapp');
  if (whatsappIcon) {
...
var maxInitialAnimationCount = Math.floor(Math.random() * 2) + 1;
