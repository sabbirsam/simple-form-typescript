...
class Tables {
	...
	public function save() {
		...
		$response = SIMPLEFORM()->database->table->update( $id, $settings );
	...
	public function create() {
		...
		if ( ! SIMPLEFORM()->helpers->is_pro_active() && SIMPLEFORM()->database->table->has( $sheet_url ) ) {
			wp_send_json_error([
				'type'    => 'sheet_exists',
		...
		$gid = SIMPLEFORM()->helpers->get_grid_id( $sheet_url );

		if ( false === $gid && SIMPLEFORM()->helpers->is_pro_active() ) {
			wp_send_json_error([
		...
		$sheet_id = SIMPLEFORM()->helpers->get_sheet_id( $sheet_url );
		$response = SIMPLEFORM()->helpers->get_csv_data( $sheet_url, $sheet_id, $gid );
	...
	public function get_table_preview() {
		...
		$sheet_id   = SIMPLEFORM()->helpers->get_sheet_id( $sheet_url );
		$sheet_gid  = SIMPLEFORM()->helpers->get_grid_id( $sheet_url );
		$styles     = [];
		...
		if ( SIMPLEFORM()->helpers->is_pro_active() ) {
			...
			$table_data = SIMPLEFORM()->helpers->get_csv_data( $sheet_url, $sheet_id, $sheet_gid );
			$response = SIMPLEFORM()->helpers->generate_html( $table_data, $settings, $table_name, false );
		}
	...
	public function table_fetch() {
		...
		$fetched_tables = SIMPLEFORM()->database->get_all();
		...
		foreach ( $fetched_tables as $table_data ) {
			$table .= '<tr>';
				...
				$table .= '<td class="text-center">';
					...
					$table .= SIMPLEFORM()->helpers->get_table_type( $table_data->source_type );
				$table .= '</td>';
	...
	public function tabNameToggle() {
		...
		$response = SIMPLEFORM()->database->update_tab_name_toggle( $id, $name );
	...
	public function get() {
		...
		$table = SIMPLEFORM()->database->table->get( $id );
		...
		$sheet_id   = SIMPLEFORM()->helpers->get_sheet_id( $table['source_url'] );
		$sheet_gid  = SIMPLEFORM()->helpers->get_grid_id( $table['source_url'] );
		$settings   = json_decode( $table['table_settings'], true );
		$settings   = null !== $settings ? $settings : unserialize( $table['table_settings'] ); // phpcs:ignore
		$styles     = [];
		...
		if ( SIMPLEFORM()->helpers->is_pro_active() ) {
			...
			$response = SIMPLEFORM()->helpers->get_csv_data( $table['source_url'], $sheet_id, $sheet_gid );
			$response = SIMPLEFORM()->helpers->generate_html( $response, $settings, $name, $from_block );
		}
	...
	public function get_plain( int $id ) {
		...
		if ( SIMPLEFORM()->helpers->is_pro_active() ) {
			...
			$response = SIMPLEFORM()->helpers->get_csv_data( $table['source_url'], $sheet_id, $sheet_gid );
			$response = SIMPLEFORM()->helpers->generate_html( $response, $settings, $name );
		}
	...
	public static function generate_table_html_for_gt( string $url, $settings, $name ) {
		$gid = SIMPLEFORM()->helpers->get_grid_id( $url );
		...
		$sheet_id = SIMPLEFORM()->helpers->get_sheet_id( $url );
		$response = SIMPLEFORM()->helpers->get_csv_data( $url, $sheet_id, $gid );
	...
	public function save_table( string $url, string $table_name, array $settings ) {
		if ( ! SIMPLEFORM()->helpers->is_pro_active() && SIMPLEFORM()->database->has( $url ) ) {
			wp_send_json_error([
				'type'   => 'sheet_exists',
		...
		$response = SIMPLEFORM()->database->table->insert( $data );
	...
	public function update_changes( int $table_id, array $settings ) {
		$settings = $this->migrate_settings( $settings );
		$response = SIMPLEFORM()->database->update( $table_id, $settings );
